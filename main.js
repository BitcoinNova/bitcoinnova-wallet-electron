...
const DEFAULT_TITLE = 'WalletShell BitcoinNova Wallet';
const DEFAULT_TRAY_TIP = 'Slow and steady wins the race!';
const PUBLIC_NODES_URL = 'https://raw.githubusercontent.com/BitcoinNova/bitcoinnova-nodes-json/master/bitcoinnova-nodes.json';
...
app.setAppUserModelId('org.bitcoinnova.walletshell');
...
function serviceBinCheck(){
    ...
    fs.copyFile(DEFAULT_SERVICE_BIN, targetPath, (err) => {
        if (err){
            ...
            log.debug(`Bitcoinnova-service copied to ${targetPath}`);
        }
        //log.warn(`BitcoinNovaService copied to ${targetPath}`);
        //settings.set('service_bin', targetPath);
    });
}
